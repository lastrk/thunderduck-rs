...
tools: [Read, Glob, Grep, LSP, mcp__codegraph__*, mcp__semble__*]
model: opus
---
...
- Verify with `codegraph_explore` (callers + blast radius in one call) before API-change asks; prefer it and `semble.search` over Grep.
