...
1. **Compute** a slug from `$ARGUMENTS`: lowercase, replace any run of
   ...
   at 40 characters. If the result is empty, use `bugfix` as the
   fallback slug.

2. **Compose** a branch name: `bugfix/<slug>-<YYYYMMDD-HHMMSS>`
...
> 2. Determine the review window. **You are running on a bugfix branch
