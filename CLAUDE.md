...
6. **Extension version pinning**: The `.duckdb_extension` binary DuckDB version must exactly match the `duckdb` crate version in `Cargo.toml`. Currently pinned to `duckdb1.5.1-ext3` (DuckDB 1.5.1, crate `1.10501.0`).
...
The `thdck_spark_funcs` DuckDB extension (release [`duckdb1.5.1-ext3`](https://github.com/lastrk/thunderduck-duckdb-extension/releases/tag/duckdb1.5.1-ext3)) implements Spark-precise numerical semantics:
