...
- The DataFrame corpus (`./tests/scripts/run-differential-tests.sh core`, 384 cases incl. the migrated `tpch`/`tpcds` clusters) is the fitness function. TPC-H/TPC-DS cases are held to the same standard as every other case: a red TPC case is a defect to fix, not tolerable background signal.
...
4. **Corpus differential** — `./tests/scripts/run-differential-tests.sh core` (DataFrame corpus, 384 cases) and `./tests/scripts/run-differential-tests.sh sql_v2` (SQL corpus, 396 cases) are the fitness gates; `./tests/scripts/differential-progress.sh` runs the whole suite once and records the progress row. Both corpora CONTAIN the TPC-H/TPC-DS clusters (`tpch-*`/`tpcds-*` case ids), held to the same standard as every other case — red TPC cases are defects to drive to green. The hard gate requirement remains "no previously-green case regresses."
...
13. **Arrow interval wire-encoding must be transcoded server-side (DuckDB is uniform, Spark is per-semantic)**: DuckDB emits every SQL `INTERVAL`-typed column as Arrow `Interval(MonthDayNano)` (16 B). Spark 4.1's Arrow wire encoding uses a different Arrow type for each of the three interval semantic kinds — `DayTimeIntervalType` → `Duration(Microsecond)` (i64), `CalendarIntervalType` → `Interval(MonthDayNano)`, `YearMonthIntervalType` → `Interval(YearMonth)` (i32). The transcode from DuckDB's uniform encoding to Spark's per-semantic encoding lives in `crates/connect-server/src/arrow_interval_transcode.rs` (`IntervalPlan::build(resolved_schema)` + `apply(batch, plan)`), runs per-batch on the tonic async task after the mpsc hop (never on the session thread — the `!Send` `Connection` isolation is preserved), and is a **connect-server concern only** — INV10 forbids τ (`crates/core/transpiler_v2/`) from importing anything runtime/wire-shape-related. Do NOT try to "fix" wire encoding inside the analyzer or emission modules; the fix is per-column data rewrite at the Arrow boundary, orchestrated by `execute_streaming_query`'s streaming loop. The paired `arrow_schema_stamp.rs` accepts both the pre- and post-transcode Arrow types at DayTimeInterval τ positions so tests + non-streaming callers keep working.

14. **`ExecutePlanResponse.schema` frame is mandatory for any interval-column result — the PySpark client's Arrow-schema fallback lacks `is_interval` arms**: PySpark's `_execute_and_fetch_as_iterator` (`pyspark/sql/connect/client/core.py`) decodes the result schema in one of two ways: (a) if the server sent a `schema` frame first, `proto_schema_to_pyspark_data_type` — which has arms for all three interval kinds — decodes it; (b) otherwise it falls back to `from_arrow_schema(table.schema, ...)`, which routes through `from_arrow_type` (`pyspark/sql/pandas/types.py`) — and that function has NO `is_interval` arm and raises `UNSUPPORTED_DATA_TYPE_FOR_ARROW_CONVERSION` on `Interval(MonthDayNano)` or `Interval(YearMonth)`. The streaming path in `execute_streaming_query` (`service.rs`) therefore yields a dedicated schema frame BEFORE any Arrow batch, built from τ's `resolved_schema` via `build_schema_response`. Any op with a data-driven or otherwise client-unfriendly Arrow schema must ride this same pattern — do not rely on the client's Arrow-schema fallback. This is orthogonal to gotcha 13: `DayTimeInterval` needs both the schema frame AND the `Duration(us)` transcode; `CalendarInterval`/`YearMonthInterval` currently fail on the client's row-decoder regardless (tracked in the corpus with `expected_error=UNSUPPORTED_DATA_TYPE_FOR_ARROW_CONVERSION`; both engines fail identically, tri-state PASS).
