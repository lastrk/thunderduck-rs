...
/// Serialize one `RecordBatch` into a proto `ArrowBatch` message.
///
/// The batch (including a 0-row schema-only batch) is written as an
/// independent Arrow IPC stream so that the client can decode it individually.
