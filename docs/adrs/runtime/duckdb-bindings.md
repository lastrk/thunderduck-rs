...
**Version pinning**: The linked DuckDB library version **must** exactly match the compiled `thdck_spark_funcs.duckdb_extension` binary version — DuckDB enforces this at `LOAD` time. The Rust API uses the crates.io `duckdb` `1.10505.0` bindings and links DuckDB's official `v1.5.5` static release archive. The crate's unused bundled source version is not the linked library version. The architecture additionally sets a floor of **DuckDB ≥ v1.5.3** wherever the Iceberg write path is used (see [ADR-016](../adr-016-version-and-ansi-pins.md)).
