...
`build.rs` downloads the platform-appropriate binary from the [`ext6` release](https://github.com/nubank/thunderduck-duckdb-extension/releases/tag/ext6) and caches it under `extensions/ext6/`. The extension is **mandatory** — failure to load is a hard startup error (see [rearchitect ADR-020](../../thunderduck-rearchitect-ADRs.md)). The extension is the `thdck_spark_funcs` DuckDB extension from the `thunderduck-duckdb-extension` repository — a C/C++ extension, platform-independent from the Rust host's perspective, compiled separately and bundled as bytes.
