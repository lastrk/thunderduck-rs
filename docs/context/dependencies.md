...
- `duckdb` crate: upstream `v1.10505.0` Git revision (the crates.io archive
  embeds v1.5.4 and cannot load v1.5.5 extensions)

All 4 platform binaries (`linux_amd64`, `linux_arm64`, `osx_amd64`, `osx_arm64`) of the adopted release are vendored — checked into git plain (uncompressed) under `extensions/vendored/` (`MANIFEST.toml` + one `.duckdb_extension` per platform), exactly one version at a time. `build.rs` picks the binary matching `TARGET` at build time and embeds it via `include_bytes!()`; extension embedding needs no network access. Clean builds download the version-matched official DuckDB library. Adopting a new release (only on `duckdb` crate bumps) is done via `scripts/dev/adopt-extension-release.sh <release-tag> <duckdb-version>` (legacy release-download mode) or `scripts/dev/adopt-extension-release.sh --from-local <dir>` (adopts locally- or CI-built binaries, regenerating the manifest's `[source]` block with in-tree provenance instead of a release tag), either of which also regenerates the manifest's per-artifact sha256/size and supports a `--verify` mode. The extension is loaded at every session's startup; failure to load is a hard error.
