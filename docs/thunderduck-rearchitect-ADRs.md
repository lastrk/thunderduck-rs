...
1. **`uniquify` field names once per row type — in the substrate name plane only** (a ~40-line utility after Calcite's `SqlValidatorUtil.uniquify`) — establishes the invariant ordinal *rendering* relies on (names unique within any row type emission generates SQL against) and kills duplicate-name ambiguity on its own. **Scope guard (ADR-005 parity):** `uniquify` never touches the analyzer's `resolved_schema` — Spark's output schemas legitimately carry duplicate names (`df.select(df1.a, df2.a)` → two columns named `a`; a self-join preserving both sides' `id`) and AnalyzePlan returns `resolved_schema` verbatim. Uniquified names live only in emitted SQL (subquery SELECT-list aliases, struct CAST fields — the existing `dedup_struct_field_names` practice generalized); the wire boundary re-imposes Spark's names via the Arrow-schema stamp, dedup'd there only per PySpark's own `_deduplicate_field_names` convention, which reference Spark itself applies.
...
**The DataFrame front-end's `plan_id` is a resolution key of the same scope machinery.** The DataFrame front-end disambiguates by `plan_id` (first-class per ADR-021: `UnresolvedColumn.plan_id`, `Join.left_plan_ids`/`right_plan_ids`), which SQL text cannot express. Under tier 3 a plan_id resolves like a qualifier: the scope maps it to a candidate field range, and (plan_id, name) binds to (input, ordinal) within that range — the same 0/1/2+ discipline applies to the plan_id binding itself. A plan_id binding disjoint left AND right ranges of one join (`df.join(df, …)`, un-realiased self-join) is a 2+ match ⇒ Spark-emulated `AMBIGUOUS_COLUMN_REFERENCE`, replacing today's silent first-match-left bind (`RelScope::lookup_plan_id`, `qualify_plan_id_refs`) — the DataFrame analogue of F11. Nested bindings of one plan_id on the same side (outermost-wins) are one binding, not ambiguity. Once plan_id binds an ordinal, the synthetic `__td_jl`/`__td_jr` machinery (`qualify_plan_id_refs`, `resolve_column`'s stamp-when-ambiguous heuristic, the `mark_join_alias_requirements` post-pass) retires with the rest of the strip machinery. A plan_id-carrying star (`df1["*"]`; `UnresolvedStar.plan_id`, currently dropped by the converter) is an ordinal *range* under the same mapping. **Scope notes:** the 0/1/2+ discipline governs expression-level references; column-SET operator arguments (`drop`/`withColumnRenamed`/`withColumns` by name) keep Spark's all-matches semantics — a name resolves to the (possibly empty, multi-element) set of ordinals, and 0 matches is a no-op, never `UnknownColumn`. Lambda parameters resolve to their binder, not to (input, ordinal).
...
- (−) Ordinal space has a real cost: offset arithmetic on every join concatenation and projection reorder. Calcite pays it with dedicated shuttles (`RexShuttle`, `rex/RexPermuteInputsShuttle.java`); τ must adopt the same discipline — a single shared remap facility, not ad-hoc index math per operator — noting the cost is smaller than Calcite's: with no optimizer (ADR-001) the tree is never rewritten under existing ordinals, so remaps occur only at fixed structural points (join concat, USING output contraction — the DataFrame default join shape, so tier 3's DataFrame coverage requires the USING output→input map — set-ops, and SelectBlock merge slots), never as tree-rewrite shuttles — and an off-by-one there is a silent wrong-column bug, the very class ADR-014's differential oracle exists to catch.
