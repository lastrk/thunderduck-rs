...
**Refinement — CommonAST carries a v2-native Expression payload (added post-Slice-D, 2026-07-02, per ADR-021).** CommonAST's expression-payload types are v2-native (`crate::transpiler_v2::expression::Expression`), NOT the legacy `crate::expression::Expression`. The two enums may hold identical initial contents (a 1:1 seed) but diverge independently — v2 can add variants, refine nullability semantics, or reshape the payload without touching legacy, and legacy expression changes cannot silently regress v2. This is the substrate-independence rule of ADR-021 applied to expression-level types. Value-level types (`DataType`, `StructType`, `StructField`) remain shared because they carry no behavior; sharing them is safe, and duplicating them would risk conversion bugs at the boundary.
...
**Refinement — dispatch is at the protobuf boundary; each path has its own front-end (added post-Slice-D, 2026-07-02, per ADR-021).** The transpiler-path dispatch (`THUNDERDUCK_TRANSPILER=legacy|v2`) is chosen **before** any protobuf conversion, in `crates/connect-server/src/service.rs`. Each path has its own protobuf-to-common-AST converter: legacy calls `PlanConverter` (producing legacy `LogicalPlan`, then `SqlGenerator` emits SQL); v2 calls `V2RelationConverter` (producing v2 CommonAST directly). Neither converter's output is consumed by the other path. The SparkSQL front-end (raw SQL text via `spark.sql(...)`) parses through the SparkSQL parser and lowers to whichever common representation the dispatched path uses. Front-end agreement (INV7) is refined accordingly: the invariant now covers the two v2 front-ends (V2RelationConverter for Spark Connect protobuf; the SparkSQL parser lowering into CommonAST for raw-SQL) as a pair that must converge to the same CommonAST for semantically equivalent inputs. Legacy is out of INV7's scope after this refinement. See ADR-021 for the substrate-independence rationale that motivates this dispatch relocation.
...
**Refinement — v2 has its own TypeInferenceEngine, not a delegating client of legacy's (added post-Slice-D, 2026-07-02, per ADR-021).** v2's type/nullability inference (`crate::transpiler_v2::type_inference::TypeInferenceEngine`) is a **peer implementation** to legacy `crate::types::TypeInferenceEngine`, not a delegating client. Both engines implement Spark semantics; ADR-015's differential oracle validates parity at test time. The rationale for two implementations rather than one shared substrate: (a) v2 can refine inference rules — add new aggregate return types, revise decimal precision propagation, close symmetric-omission gaps — without touching legacy, and vice versa; (b) bugs in one engine cannot silently transit into the other (concrete precedent: Slice D Phase 2's corr/covar_samp/regr_* symmetric-omission was a legacy TypeInferenceEngine gap that would have been invisible-until-red under a shared substrate; independent engines make the failure mode local); (c) ADR-015's AnalyzePlan diff is *already* the correctness contract for Spark parity — sharing code adds coupling without adding correctness (the oracle IS the parity mechanism, not the source code). The seed strategy is a 1:1 initial copy of legacy's engine (aggregate roster, coercion lattice, nullability derivations, decimal formulas — verbatim); divergence is bounded by the ADR-015 oracle and by the ADR-003 incremental-extension rule.
...
**Refinement — the contamination barrier extends to the input side (added post-Slice-D, 2026-07-02, per ADR-021).** The two-decision-spaces barrier is not only "v2 emission does not import legacy `SqlGenerator` / `FunctionRegistry`" (the emission-side rule that INV3 codifies). It also covers the **input side**: v2 does not consume legacy `LogicalPlan` as its input type, does not carry legacy `Expression` in the CommonAST payload, does not delegate to legacy `TypeInferenceEngine`. The only substrate shared between v2 and legacy is value-level types (`DataType` / `StructType` / `StructField`) — pure data with no behavior. The input-side barrier is codified as INV10 (see §CV.5), a peer to INV3's emission-side barrier. Together they bracket the two decision spaces: the dispatch site chooses one or the other, and from that point onward the two paths share only value-level types and the ADR-015 differential oracle (which validates parity by test, not by shared code). See ADR-021 for the full architectural commitment; see LB9 in §CV.4 for the load-bearing assumption that two independent implementations validated by ADR-015 deliver equivalent Spark semantics with less risk than one shared implementation.
...
**Refinement hooks.** Specify the result-canonicalization rules precisely (the float/NaN/decimal/collation/map handling). Build the AnalyzePlan schema-diff first, as validation harness, inference implementation oracle (ADR-005/006), and front-end-agreement check (INV7). Define the catalog fixture and how it is established identically on both engines.

**Refinement — one source of truth per implementation, not shared across implementations (added post-Slice-D, 2026-07-02, per ADR-021).** ADR-015's differential oracle validates parity *between implementations* — it does **not** mandate code sharing between them. The universe of DataFrame expressions and SparkSQL has one source of truth *inside each implementation*: legacy's `TypeInferenceEngine` is the source of truth for legacy; v2's `TypeInferenceEngine` is the source of truth for v2. Parity is a test-time property enforced by the AnalyzePlan schema diff + the result differential, not a compile-time property enforced by shared code. This reframing has two consequences: (1) two implementations of Spark semantics is *not* duplication in the load-bearing sense — it is two independently-validated engines, and the oracle IS the parity contract; (2) a bug in one engine cannot silently transit to the other (a real risk under shared substrate, and the empirical motivation for ADR-021's substrate independence). See LB9 in §CV.4 for the load-bearing form of this reframing.

---

## ADR-016 — Pinned reference version; coverage claims are version-scoped

**Status:** Proposed
**Depends on:** ADR-010
**Depended on by:** ADR-014, ADR-015, ADR-017, ADR-018

**Context.** Spark's semantics (type coercion, nullability, function behavior, SQL grammar) evolve across versions. Coverage claims and inference fidelity are only meaningful against a fixed semantics.

**Decision.** Everything is pegged to Apache Spark 4.1.1. The pinned-artifact set also includes DuckDB and its extensions, with a floor of **DuckDB ≥ v1.5.3** where the Iceberg write path (ADR-018) is used (required for MERGE, ALTER, and Iceberg v3 deletion vectors). A version bump re-derives the coverage denominators (ADR-009, ADR-014) and re-runs both suites (ADR-015). The pinning policy is bump-and-re-run, with the pinned version a hard CI check against the Spark image under test.

**Consequences.**
- (+) Coverage claims and parity guarantees are precise and auditable against a definite semantics.
- (−) A version bump is a planned, multi-day effort (regenerate denominators, re-run suites, reconcile any new divergences in coercion/nullability/SQL-grammar), not a silent change.
- (neutral) A `SparkVersion` constant documents the contract and preserves room for version-conditional behavior if ever needed.

**Refinement hooks.** Establish the version-bump runbook. Decide whether any transformation ever needs to be version-conditional (default: none; all unconditional on the pinned version).
...
## ADR-021 — v2 substrate independence: dispatch at the protobuf boundary, no shared Expression, no shared TypeInferenceEngine, no legacy LogicalPlan input
...
**Depended on by:** every v2 implementation slice (see readiness map)

**Context.** The additive-landing pattern (ADR-002 refinement, added post-Slice-C) let v2 emerge as a *downstream* consumer of the legacy `LogicalPlan`. The dispatch point sits in `crates/connect-server/src/service.rs` **after** the legacy `PlanConverter` has already deserialized the Spark Connect protobuf into a legacy `LogicalPlan`; v2's `crates/core/src/transpiler_v2/lowering.rs` then adapts that `LogicalPlan` into the common AST, and the v2 analyzer delegates all expression type/nullability calls into the legacy `crate::types::TypeInferenceEngine` and the legacy `crate::expression::Expression::{data_type, nullable}` methods. The CommonAST itself carries the legacy `Expression` enum as its expression payload. INV3's grep barrier (v2 emission does not `use crate::generator::*` / `use crate::functions::*`) is hygienic on the *emission* side, but it is silent on the *input* side — and that silence is where the substrate coupling lives.

The empirical symptom of this coupling is the "dormant v2 fix" pattern, now four-instance-anchored: C.3-1 (`hash-002`), C.3-6b (`agg-013`), Slice D Phase 2's `math-016`, and Slice D Phase 2's `agg2-004`. In every case, the v2 emission arm is correct, unit-tested, and INV-preserving, yet the corpus stays red because the runtime routes through legacy fallback — for reasons that trace to the `RelationConverter` producing shortcut shapes (`SqlRelation`-wrapped synthesized SQL from Arrow-IPC `createDataFrame`, plan-id-encoded qualifiers on `UnresolvedColumn`, `AggregateSelectOrder` variants) that v2's `lowering.rs` cannot lower onto the CommonAST surface without inheriting the shortcut. The `SqlRelation` audit (2026-07-02) confirmed: 16 construction sites, zero of which represent a genuinely opaque raw-SQL scenario — every single one is either (a) a synthesized-SQL shortcut for a structured plan the converter *could* have built, (b) a catalog metadata query that arguably should not be a plan node at all, or (c) a test artifact. v2 inherits every one of these shortcuts by construction.

An audit of the v2 module tree (also 2026-07-02) quantified the coupling: 32 cross-module imports from `crate::logical` / `crate::expression` / `crate::types::TypeInferenceEngine`; ~260 mentions of `LogicalPlan` / `Expression` in `transpiler_v2/` source; and `~1000 LOC` of `RelationConverter` proto-handling logic that v2 transitively depends on. INV3 is perfect. The coupling is nonetheless structural.

**Decision.** v2 is a **peer** to legacy from the protobuf boundary onward, not a downstream consumer of it. Concretely:

1. **Dispatch at the protobuf boundary.** The `THUNDERDUCK_TRANSPILER` selection (`legacy` | `v2`) is made in `service.rs` **before** any protobuf-to-plan conversion. Each path invokes its own converter: legacy calls `PlanConverter` (producing legacy `LogicalPlan`); v2 calls a new `V2RelationConverter` (producing v2 CommonAST directly). No path consumes the other's plan representation.

2. **v2-native Expression.** `crate::transpiler_v2::expression::Expression` is a **new type**, not `crate::expression::Expression`. The two enums may start with identical contents (a 1:1 initial copy is the sane migration seed) but diverge independently. v2 can add variants, refine nullability, or reshape the expression surface without touching legacy — and vice versa.

3. **v2-native TypeInferenceEngine.** `crate::transpiler_v2::type_inference::TypeInferenceEngine` is a **new implementation**, not a delegating client of `crate::types::TypeInferenceEngine`. Both engines implement Spark semantics; ADR-015's differential oracle validates their parity at test time. Code sharing between the two engines is explicitly **not** the parity mechanism.

4. **Value-level types remain shared.** `DataType`, `StructType`, `StructField` stay in `crate::types::*` and are used verbatim by both paths. These are pure value types (no behavior); duplicating them would risk conversion bugs at the boundary without buying substrate independence.

5. **`transpiler_v2/lowering.rs` is deleted.** Its purpose (adapting legacy `LogicalPlan` to CommonAST) evaporates once `V2RelationConverter` produces CommonAST directly. All 16 `SqlRelation` construction sites are either replaced by structured CommonAST variants inside `V2RelationConverter` (for Cat-A synthesized-SQL cases: `Values`, `TableFunction`/`Unnest`, `FileScan`, `LocalRelation` for Arrow-IPC) or routed as service-layer library calls rather than plan nodes (for Cat-D catalog metadata operations) — no CommonAST `Sql` opaque variant is needed because the audit found no genuinely-opaque residual class.

**Alternatives considered (and rejected).**
- *Keep current downstream dispatch, treat coupling as documented tax.* **Rejected.** Four dormant-v2-fix instances prove the tax is not small; every new Spark Connect proto extension transits through legacy first; every RelationConverter bug affects v2 identically to legacy.
- *Share `Expression` and `TypeInferenceEngine`; isolate only `RelationConverter`.* **Rejected.** Still leaves v2 unable to refine the expression surface without touching legacy; every legacy Expression bug still transits into v2; the "one implementation validated by tests" argument is met by having ADR-015's oracle, not by shared code.
- *Full substrate independence (this ADR).* **Chosen.** v2 has its own protobuf converter, its own Expression enum, its own TypeInferenceEngine. Value-level types (`DataType`/`StructType`) shared only.

**Consequences.**
- (+) The four in-flight dormant-v2-fix instances activate immediately once V2RelationConverter lands; the dormant-fix pattern class disappears entirely for future work (there is no legacy fallback path where v2 could be silently correct).
- (+) v2 can refine its expression surface — add new variants, revise nullability, reshape payload — without touching legacy. And vice versa: legacy expression changes cannot regress v2.
- (+) TypeInferenceEngine bugs are found and fixed independently in each engine; ADR-015's differential harness catches any parity drift.
- (+) v2 does not inherit RelationConverter's quirks: no `SqlRelation` shortcut, no plan-id qualifier encoding, no LocalRelation-as-VALUES-SQL synthesis, no silent-NULL Arrow marshalling bugs like C.3-4.
- (+) INV3's grep barrier extends naturally to input-side imports (see INV10 below): `use crate::logical::*` / `use crate::expression::*` / `use crate::types::TypeInferenceEngine` from `transpiler_v2/` all become forbidden.
- (−) Substantial new code: `V2RelationConverter` (~1000 LOC mirroring `RelationConverter`'s proto surface), v2 `Expression` enum, v2 `TypeInferenceEngine`, v2 `spark_column_name` utility.
- (−) Two implementations of Spark type/nullability semantics — validated only by ADR-015's differential oracle. If the oracle has coverage gaps, divergence can persist.
- (−) Reshapes the substrate assumptions of every already-landed v2 slice (A/B/C/D). No slice's *concepts* are scratched — the CommonAST shape, the three-pass analyzer structure, the emission-table dispatch, the extension_targets allow-list all survive — but the *type parameters* of the substrate change from legacy to v2-native. See the readiness-map §Slice B0 addition for the migration plan.
- (−) Two source-of-truth engines for Spark semantics is a documentation and cognitive tax; a change to Spark's coercion lattice or nullability rules must be applied in both.

**Refinement hooks.**
- **V2RelationConverter's protobuf surface** must be exhaustive over the Spark Connect proto set that v2 targets. Any un-handled proto message shape is a typed `Punt` that routes to legacy per the additive-landing model of ADR-002 — not a silent black-box.
- **V2 `Expression` enum seed.** The initial contents should be a 1:1 copy of legacy `Expression`'s 21 variants; divergence is bounded by the corpus test surface (add variants only for cases the corpus needs, per ADR-003's incremental-extension rule).
- **V2 `TypeInferenceEngine` seed.** Same seed strategy: a 1:1 copy of legacy `TypeInferenceEngine` (the aggregate-return-type table, the coercion lattice, the nullability rules) as the migration start. Divergence is bounded by ADR-015's parity oracle.
- **INV3 grep tightening.** With v2 no longer consuming `LogicalPlan` / `Expression` / `TypeInferenceEngine`, the grep barrier in `crates/core/src/transpiler_v2/invariants.rs` extends to reject those imports as well. INV10 (below) codifies the check.
- **Cost-recouping.** The one-time cost is bounded (roughly one dedicated slice — "Slice B0" in the readiness map). The ongoing cost is bounded by the parity oracle (already the test harness). The dormant-v2-fix and RelationConverter-bug-transit costs are eliminated as recurring taxes.
...
The twenty decisions are not peers; they form four strata.

**Premise (the selector):** ADR-000 (positioning). It is logically prior to everything: it selects Alternative 2 (Rust reimplementation), which is *why* thunderduck owns an analyzer at all. A change here (e.g. accepting a JVM in the path) would cascade further than any other single ADR — it could delete ADR-005/006 outright.

**Spine (the irreducible commitments, given the premise):** ADR-001 (transliterator), ADR-002 (emit-level delegation), ADR-005 (own the divergent slice). These define what τ *is* and what it *owns*.

**Substrate & front-ends (the IR and how it is populated and analyzed):** ADR-003 (common AST), ADR-004 (both front-ends lower to it; relation-vs-command by parse-root), ADR-006 (analyzer pass structure). These define the representation everything else operates on.

**Consequences (the spine applied to surfaces):** ADR-007 (A/B/C, B retained) from 001+002+005+006+008; ADR-008 (correlated direct) from 001; ADR-009 (declarative table, compiled dispatch) from 003+005+007; ADR-010 (extension fns, C++ project) from 005; ADR-011 (commands) from 001+004; ADR-012 (catalog overlay) from 011+005; ADR-013 (external/lakehouse reads, delegated) from 000+002+012; ADR-017 (Delta append writes) and ADR-018 (UC-managed Iceberg CTAS/INSERT/DELETE/MERGE writes) — the per-format write specializations — both from 005+011+013+016; ADR-019 (the native-read / Iceberg-write lakehouse I/O contract) composing 013+018.
...
| 021 v2 substrate independence | 002, 003, 004, 005, 014, 015 | every v2 implementation slice; refines the input-side dispatch boundary of ADR-004 and reshapes ADR-005's "one owned analyzer" as one-per-implementation |
...
**T4 — Delegation assumption vs its own validation (ADR-002 ↔ ADR-015).** ADR-002 assumes "DuckDB's structural resolution matches what's wanted," but that assumption is only *validated* by the test architecture ADR-002 ultimately enables. **Resolution:** not a conflict but a reinforcing loop to make explicit — the AnalyzePlan schema diff (ADR-015) on starred/ambiguous/scope-heavy plans is what empirically confirms the delegation boundary (LB3). Treat delegation as provisional until that diff is clean.

**T5 — Two front-ends vs one representation (ADR-003 ↔ ADR-004).** The common-AST correctness guarantee ("SQL constructs that map to existing nodes inherit emission/inference for free") holds only if the SQL parser and the Connect deserializer produce the *same* node for the same semantics — but thunderduck has two independent front-ends where Spark has one shared `AstBuilder`. **Resolution:** INV7 makes front-end agreement a preserved invariant, checked by the AnalyzePlan schema diff (ADR-015) on the same SQL parsed by thunderduck and analyzed by Spark. If the two front-ends diverge on a shared construct, the guarantee breaks for that construct until reconciled.

**T6 — Substrate independence vs shared correctness ground truth (ADR-005 ↔ ADR-021).** ADR-005 says thunderduck *owns* Spark type/nullability inference — one owned analyzer that gets the divergent slice right. ADR-021 says v2 has its own analyzer, separate from legacy's. Superficially this looks like duplication — two implementations of the same "owned" semantics. **Resolution:** the ADRs are not in conflict; they operate on different senses of "one." ADR-005 says there is one owned analyzer *per implementation*, not one shared across implementations. Legacy has one owned analyzer (`crate::types::TypeInferenceEngine`); v2 has one owned analyzer (`crate::transpiler_v2::type_inference::TypeInferenceEngine`). ADR-015's differential oracle enforces cross-implementation parity at test time — it IS the correctness contract, not the shared code. This trades a compile-time coupling (shared substrate) for a test-time coupling (oracle-validated parity), which is the deliberate cost that buys the substrate independence ADR-021 requires. Watch this tension when reviewing changes that would collapse the two engines into one — the argument "it's the same semantics, why have two?" is only correct if you also accept re-introducing the coupling class that ADR-021 removes. See LB9 for the load-bearing form of the trade-off.
...
**LB3 — DuckDB's structural resolution (binding, star, scope) matches what's wanted.** If false, ADR-002's delegation breaks for those constructs and they must move into the owned slice. **Check:** AnalyzePlan schema diff on starred / ambiguous-column / deep-scope plans (Tension T4).
...
**LB9 — Two independent implementations of Spark type/nullability semantics, validated by ADR-015's differential oracle, deliver equivalent correctness with strictly less coupling than one shared implementation.** (From ADR-021.) The intuition is straightforward: shared substrate creates a coupling class — a bug in legacy `TypeInferenceEngine` transits identically into v2; a refactor of legacy `Expression` forces v2 changes; a schema-inference gap discovered while implementing v2 requires touching legacy to fix. Under two independent implementations, each engine is fixed independently, and ADR-015's oracle catches any parity drift at test time. The empirical precedent is Slice D Phase 2's symmetric-omission on `corr`/`covar_samp`/`regr_*` (Spark returns `Double`; legacy's `aggregate_return_type` fell through to argument type, giving `Integer` on integer-column corpus cases) — under the previous shared-substrate model, that gap silently affected both paths identically until the corpus differential surfaced it. Under LB9, each engine has its own aggregate roster, and the oracle validates parity at test time. **Check:** the ADR-015 differential harness *is* the parity check — an unbounded corpus of DataFrame + SparkSQL cases, each routed through both implementations, results diffed. If the harness has coverage gaps, LB9 can fail silently for those gaps; the mitigation is the same as LB5's — grow the harness, or reject the case class explicitly. Failure mode is silent divergence on un-covered cases, which is exactly what ADR-015 is designed to prevent — so LB9's soundness rests on ADR-015's coverage discipline.
...
**INV7 — Both v2 front-ends produce the same v2 common-AST node for semantically equivalent inputs.** (Added with the common-AST/SQL ADRs; refined post-Slice-D 2026-07-02 per ADR-021 to be explicit about v2 scope; touches ADR-003, ADR-004, ADR-021.) The two v2 front-ends — `V2RelationConverter` (Spark Connect protobuf → CommonAST) and the SparkSQL parser lowering into CommonAST for `spark.sql(...)` input — must normalize to identical AST (same node, same resolved type/nullability) for the same meaning; otherwise the common-AST guarantee — that SQL inherits emission/inference rules for free for shared constructs — breaks. This is the soundness condition for having one τ behind two front-ends *within v2*. Legacy is out of INV7's scope: it uses different substrate types entirely (legacy `LogicalPlan`, legacy `Expression`, legacy `TypeInferenceEngine`), and cross-implementation parity is validated by ADR-015's differential oracle, not by INV7. **Check:** AnalyzePlan schema diff on the same SQL routed through both v2 front-ends and compared against reference Spark (Tension T5).
...
**INV10 — v2 substrate independence: v2 does not import legacy `LogicalPlan`, `Expression`, or `TypeInferenceEngine` at runtime.** (Added with ADR-021, 2026-07-02; touches ADR-003, ADR-004, ADR-005, ADR-014, ADR-021.) The `crates/core/src/transpiler_v2/` module tree and `crates/connect-server/src/converter/v2_relation_converter.rs` share only value-level types (`DataType` / `StructType` / `StructField`) with the legacy path. v2's `Expression` enum, `TypeInferenceEngine`, and protobuf converter are peer implementations, not delegating clients. This is the *input-side* complement to INV3's *emission-side* single-source-of-truth rule; together INV3 + INV10 bracket the two decision spaces (ADR-014) from dispatch to emission. **Check:** `git grep -E 'use crate::(logical|expression)::|use crate::types::TypeInferenceEngine' crates/core/src/transpiler_v2/ crates/connect-server/src/converter/v2_relation_converter.rs` returns zero once ADR-021 lands (the "Slice B0" migration in the readiness map). Between now and that landing, the grep is transitionally non-zero; the invariant is *stub-marked* per §CV.5.1's `TODO INV10:` / `DEFER INV10 → <slice>:` convention. Note: `use crate::types::{DataType, StructType, StructField}` remains permitted — those are value-level types shared across the boundary.
...
6. **ADR-020** (strict-only extension) and **ADR-021** (v2 substrate independence) — the two post-Slice-D architectural commitments. ADR-020 first (it consolidates the emission target); ADR-021 second (it consolidates the substrate boundary). Ratifying ADR-021 confirms LB9 and INV10, and reshapes how every subsequent implementation slice reads Tension T6 and Refinement — one source of truth per implementation on ADR-015.
