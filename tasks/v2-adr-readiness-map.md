...
- **C.3-6 — `percentile_approx` / `median` shape verification. HALT-AND-FLAG 2026-07-01; +0 delta.** Preflight showed `agg-013` RED (not GREEN as the plan predicted). Root cause: DuckDB's `approx_quantile` requires `FLOAT` for the quantile arg but v2 emission passes `0.5::DOUBLE`. Emission-side bug, does not match C.3-6's verify-only shape. Out of scope; tracked as **C.3-6b (follow-up)** — needs a fresh `/fix-bug` invocation to correct the FLOAT/DOUBLE quantile-arg literal emission.

**Unlocks the Slice D Phase 1 target case IDs** (`hash-001..003`, `agg-007`, `agg-013`, `agg-020`, `agg2-006`, `math-011`, `type-005`) plus indirect unlocks. C.3-4 delivered +15; C.3-3 delivered +2; C.3-5 verify-only +0; C.3-1 dormant +0; C.3-2 +1; C.3-6 HALT-AND-FLAG. **Cumulative Slice C.3 delta (open passes): 134 → 152 (+18).** `hash-002` (blocked on Slice D/E `SqlRelation` handling) and `agg-013` (blocked on C.3-6b FLOAT quantile-arg fix) remain the outstanding Phase 1 target case IDs.
...
**Phase 1 progress signal target: 134 → ~140-142.** Post-halt-and-flag delta update (2026-07-01): **134 → 152 after Slice C.3-4 + C.3-3 + C.3-2 landed (C.3-5 verify-only, C.3-1 dormant, C.3-6 HALT-AND-FLAG)** — C.3-4 delivered +15 (`LocalRelation` Decimal marshalling), C.3-3 delivered +2 (`count_if` symmetric-omission fix), C.3-2 delivered +1 (`hash-003` via `FunctionCall::nullable` non-nullable list extension for `hash`/`murmur3`/`xxhash64`). The halt-and-flag audit blocker is **substantially resolved**; two Phase 1 target case IDs remain: `hash-002` (dormant — v2 fix landed but corpus routes through legacy `SqlRelation` fallback, blocked on future Slice D/E `SqlRelation` handling) and `agg-013` (blocked on C.3-6b FLOAT/DOUBLE quantile-arg fix — needs a follow-up `/fix-bug`).
