...
- `cast-012` (`try_cast` → native `TRY_CAST`; **already wired** at `emission.rs:1167-1168` via `Cast::try_cast` flag — needs corpus regression test to confirm green).
- `agg-012` (`corr`/`covar_samp` → native `CORR`/`COVAR_SAMP`; **needs wiring**).
- `agg2-003` (`regr_slope`/`regr_r2` → native `REGR_SLOPE(y,x)`/`REGR_R2(y,x)`; **needs wiring**).
- `agg-009 kurtosis` (`kurtosis` → native `KURTOSIS_POP` — population/excess, not DuckDB's sample-based `KURTOSIS`; **already wired** at `emission.rs:1702` — needs corpus regression test).
- `agg2-006 count_if` (`count_if` → native `COUNT_IF`; **already wired** at `emission.rs:1708` via C.3-3 landing — needs corpus regression test to confirm no ext6-rebuild regression).

**Phase 2 progress signal target: 152 → ~157-159** (contingent on per-case native-parity confirmation during Pass 1 verification).

**Cumulative Slice D progress signal target: 134 → ~157-159.** No external dependency remains; the 2026-07-01 audit confirmed 3 spark_* shipped in ext6 (`spark_try_divide`, `spark_try_sum`, `spark_try_avg`) and 7 originally-planned functions resolve natively (`TRY_CAST`, `CORR`, `COVAR_SAMP`, `REGR_SLOPE`, `REGR_R2`, `KURTOSIS_POP`, `COUNT_IF`). **INV6 gains partial teeth in Phase 1** (over the ext4-set subset) and **full teeth in Phase 2** (over the 9-entry allow-list once the 3 ext6-arms are added to `extension_targets()`).
