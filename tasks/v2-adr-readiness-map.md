...
**Phase 1 progress signal target: 134 → ~140-142.** Post-halt-and-flag delta update (2026-07-01): **134 → 149 after Slice C.3-4 landed** (the `LocalRelation` Decimal marshalling fix that closed `type-005`/`type-003`/`type-004` plus ~12 other silently-NULL'd decimal-payload cases). The halt-and-flag audit blocker is **partially resolved**; the remaining C.3-1/2/3/5/6 fixes still gate the Phase 1 hash/count_if/sum/percentile target cases.
